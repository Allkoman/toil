...
class AWSProvisioner(AbstractProvisioner):
    ...
    def _coreSSH(cls, nodeIP, *args, **kwargs):
        ...
        if appliance:
            ...
            commandTokens += ['docker', 'exec', '-i', ttyFlag, 'leader']
    ...
    @classmethod
    def _rsyncNode(cls, ip, args, applianceName='leader'):
    ...
    def _getLeader(cls, clusterName, wait=False, zone=None):
        ...
        if wait:
            ...
            cls._waitForNode(leader, 'leader')
        return leader
    ...
    def _propagateKey(self, instances):
        ...
        for node in instances:
            ...
            ipAddress = self._waitForNode(node, 'worker')
            self._rsyncNode(ipAddress, [self.config.sseKey, ':' + self.config.sseKey], applianceName='worker')
